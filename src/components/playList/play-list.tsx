...
import { PlayFilterOptions } from "./play-filter-option";
...
export const PlayListContext =
  createContextId<PlayListContextProp>("play-list-context");
      ...
      <div class={" bg-gray-100 rounded-t-2xl "}>
        ...
        <div class={"p-6 flex flex-col gap-2"}>
          <Resource value={resourceData}
            onPending={() => <>Cargando...</>}
            onRejected={() => <>Error!</>}
            onResolved={(data) => data.map((i: any) => <PlayListItem key={crypto.randomUUID()} {...i} />)}
          />
        </div>
      </div>
    </div>
